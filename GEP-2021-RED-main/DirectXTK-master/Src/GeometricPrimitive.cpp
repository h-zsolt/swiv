...
void GeometricPrimitive::Impl::Initialize(ID3D11DeviceContext* deviceContext, const VertexCollection& vertices, const IndexCollection& indices)
{
    if (vertices.size() >= USHRT_MAX)
        throw std::exception("Too many m_vertices for 16-bit index buffer");
    ...
    const std::vector<uint16_t>& indices)
{
    ...
    if (vertices.empty() || indices.empty())
        throw std::exception("Requires both m_vertices and indices");
    ...
    for (auto it = indices.cbegin(); it != indices.cend(); ++it)
    {
        if (*it >= nVerts)
        {
            throw std::exception("Index not in m_vertices list");
        }
    }
