...
void SpriteBatch::Impl::RenderBatch(ID3D11ShaderResourceView* texture, SpriteInfo const* const* sprites, size_t count)
{
    ...
    while (count > 0)
    {
        ...
        auto m_vertices = static_cast<VertexPositionColorTexture*>(grfxMemory);
#else
    ...
    FXMVECTOR inverseTextureSize)
{
    ...
    // Scale the m_origin offset by source size, taking care to avoid overflow if the source region is zero.
    ...
    // The four corner m_vertices are computed by transforming these unit-square positions.
    ...
    // Generate the four output m_vertices.
