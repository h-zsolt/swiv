    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor && state->CursorFollow)
        {
            ...
            if (is_multiline)
            {
                ...
                draw_pos.y += (draw_window->Scroll.y - scroll_y);   // Manipulate cursor m_position immediately avoid a frame of lag
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    if (menu_is_open)
    {
        SetNextWindowPos(popup_pos, ImGuiCond_Always); // Note: this is super misleading! The value will serve as reference for FindBestWindowPosForPopup(), not actual m_position.
        menu_is_open = BeginPopupEx(id, flags); // menu_is_open can be 'false' when the popup is completely clipped (e.g. zero size display)
    }
    else
    {
...
bool    ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags)
{
    ...
    // Set cursor m_position in a way which only be used in the off-chance the user erroneously submits item before BeginTabItem(): items will overlap
    window->DC.CursorPos = ImVec2(tab_bar->BarRect.Min.x, tab_bar->BarRect.Max.y + tab_bar->ItemSpacingY);
